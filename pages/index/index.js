...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.inviteCode !== undefined) {
      console.log("短信拉起CODE:", options.inviteCode)
      // 短信唤起登录前清空所有缓存
      wx.clearStorageSync()
      // 传递邀请码
      this.setData({
    ...
    if (app.globalData.platform === 'windows' || app.globalData.platform === 'mac') {
      wx.reLaunch({
  ...
  getInputId (event) {
    ...
    // 正则表达式设置只能输数字和去掉空格
    this.setData({
      talk_id: event.detail.trim()//.detail.trim().replace(/[^0-9]/g, "")
    })
  },
  getInputName (event) {
    // event.detail 为当前输入的值
    ...
    // 真实姓名的输入内容，不能输入特殊字符
    this.setData({
      username: event.detail.trim().replace(/[^0-9a-zA-Z·\u4e00-\u9fa5]/g, "")
    })
  },
  // 前端生成一个随机32位字符串（暂时弃用）
  randomString () {
